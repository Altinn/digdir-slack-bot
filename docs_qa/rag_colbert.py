...
async def rag_with_colbert(user_input):

    durations = {
        'generate_searches': 0,
        'colbert_index_search': 0,
        'rag_query': 0,
        'total': 0
    }
    ...
    print(f'Starting ColBERT search...')
    start = timeit.default_timer()
    search_hits = RAG.search(user_input)
    durations['colbert_index_search'] = round(timeit.default_timer() - start, 1)

    print(f'ColBERT search results produced in {durations["colbert_index_search"]}:')
    ...
    while doc_index < len(search_hits):        
        ...
        loaded_doc = {
            ...
            'metadata': {            
                'source': 'https://docs.altinn.studio'
            }
        }    
        print(f'loaded markdown doc, length= {len(doc_trimmed)}')
    ...
    durations['download_docs'] = round(timeit.default_timer() - start, 1)
    ...
    prompt = ChatPromptTemplate.from_messages(
            [('system', 'You are a helpful assistant.'),
             ('human',  qa_template(userInputLanguageName))]
        )
    ...
    if translation_enabled and rag_success and  userInputLanguageCode != 'en':
        translated_answer = await translate_to_language(
            result['function'].helpful_answer, userInputLanguageName)
    ...
    response = {
        ...
        'english_user_query': user_input, #extract_search_queries.questionTranslatedToEnglish,
        'user_query_language_code': userInputLanguageCode,
        'user_query_language_name': userInputLanguageName,
